...
opkg_test_install(){
	...
	[ "$1" = "vsftpd" ] && {
		...
		echo -e "\n$PINK请输入要设置的 FTP 监听端口$RESET" && ftpnum="" && ftpnum=""
		echo "---------------------------------------------------------"
...
sda_install(){
	...
	if [ -z "$(echo $1 | grep -oE 'aria2|vsftpd|transmission')" ];then
		...
		[ "$1" = "transmission" ] && {
			...
			uci set transmission.@transmission[0].rpc_host_whitelist="*.*.*.*" #"127.0.0.1,$(ip route | grep br-lan | awk {'print $9'} | awk -F '.' '{print $1"."$2"."$3".*"}')"
			...
			uci set transmission.@transmission[0].rpc_whitelist="*.*.*.*" #"127.0.0.1,$(ip route | grep br-lan | awk {'print $9'} | awk -F '.' '{print $1"."$2"."$3".*"}')"
