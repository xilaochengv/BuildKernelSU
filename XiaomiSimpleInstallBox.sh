	...
	[ "$1" = "vsftpd" ] && {
		...
		[ "$locals" = "YES" ] && {
			...
			while [ ! "$username" ];do
				...
				[ "$(echo $username | sed 's/[0-9a-zA-Z]//g')" -o ! "$username" ] && username="" && continue
		...
		while [ "$(pidof $1)" ];do killpid $(pidof $1 | awk '{print $1}');done && firewalllog "del" "$1" && runtimecount=0 && /etc/init.d/$1 start &> /dev/null
	...
	[ "$1" = "wakeonlan" ] && {
		...
		if [ "$num" != 3 ];then
			...
			[ ! "$devip" ] && {
				...
				[ ! "$devip" ] && devip=159 && while [ "$(ip neigh | grep $devip)" ];do let devip++;done && devip="$(uci get network.lan.ipaddr | sed 's/[0-9]*$//')$devip"
			}
			if [ "$num" = 1 ];then
	...
	if [ ! "$sdadir" ];then
		if [ ! "$del" ];then
			...
			while [ ! "$num" ];do
				...
				[ "$skipdownload" ] && mkdir -p $sdadir && ln -sf /usr/bin/zerotier-one $sdadir/$2
			done
		else
...
domainblacklist_update(){
	[ -f /etc/domainblacklist ] && [ "$(grep $devmac /etc/domainblacklist)" ] && {
		echo -e "\n$SKYBLUE$devmac$GREEN$devname$YELLOW当前已添加网页黑名单：$RESET"
		echo "---------------------------------------------------------"
		sed -n /$devmac/p /etc/domainblacklist | awk '{print NR":'$SKYBLUE'\t"$2"'$RESET'"}'
		echo "---------------------------------------------------------"
		[ "$(grep $devmac /etc/domainblacklist)" ] && echo -e "$PINK删除请输入 ${YELLOW}-ID $PINK，如：${YELLOW}-2$PINK 删除第二条已添加网页黑名单$RESET"
	}
	...
	case "$num" in
		...
		7)
			...
			while [ ! "$domain" ];do
				...
				[ "${domain:0:1}" = "-" ] && {
					[ "$(echo ${domain:1:$$} | sed 's/[0-9]//g')" -o ! "$(grep $devmac /etc/domainblacklist 2> /dev/null)" ] && echo -e "\n$RED输入错误！请重新输入！$RESET" && domain="" && continue
