...
github_download(){
	...
	for MIRROR in $MIRRORS;do
		...
		wait;rm -f /tmp/XiaomiSimpleInstallBox_http_code
		if [ "$size" -a "$(ls -l /tmp/$1 2> /dev/null | awk '{print $5}')" = "$size" ];then
			break
		else
			rm -f /tmp/$1
			[ ! "$3" ] && {
...
sda_install_remove(){
	sdalist=$(df | sed -n '1!p' | while read LINE;do [ "$(echo $LINE | awk '{print $4}')" != 0 ] && echo $LINE | awk '{print $6}' | grep -vE "^/$|rom|tmp|ini|overlay|etc|sys|lib|docker_disk|dev|/userdisk/";done)
		...
		[ ! "$del" ] && {
			...
			[ "$1" = "homeassistant" ] && {
				...
				if [ ! -d $sdadir/homeassistant/custom_components/hacs ];then
					...
					hav=$(curl -skm 5 "https://hub.docker.com/v2/repositories/homeassistant/aarch64-homeassistant/tags?page_size=25" | grep -oE '20[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{1,2}\.dev[0-9]{12}' | head -1)
					if [ "$hav" ];then
						echo -e "\n$YELLOW获取 Home-Assistants $hav 镜像 ······$RESET"
						docker pull homeassistant/aarch64-homeassistant:$hav
					else
						echo -e "\n$RED获取 Home-Assistants 版本号失败！" && sleep 2 && main
					fi
					...
					echo -e "\n$YELLOW启动 Home-Assistants ······$RESET" && docker run -d --name Home-Assistants --restart=unless-stopped --privileged --network=host -e TZ=Asia/Shanghai -v $sdadir/homeassistant:/config homeassistant/aarch64-homeassistant:$hav &> /dev/null
					...
					hav=$(docker images | grep homeassistant/aarch64 | awk '{print $2}' | head -1)
					[ ! "$hav" ] && hav=$(curl -skm 5 "https://hub.docker.com/v2/repositories/homeassistant/aarch64-homeassistant/tags?page_size=25" | grep -oE '20[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{1,2}\.dev[0-9]{12}' | head -1)
					docker run -d --name Home-Assistants --restart=unless-stopped --privileged --network=host -e TZ=Asia/Shanghai -v $sdadir/homeassistant:/config homeassistant/aarch64-homeassistant:$hav &> /dev/null
	...
	[ "$del" ] && {
		...
		[ "$1" = "homeassistant" ] && {
			...
			hav=$(docker images | grep homeassistant/aarch64 | awk '{print $2}' | head -1)
			[ "$hav" ] && echo -e "\n$YELLOW正在删除 Home-Assistants ······$RESET" && docker rmi homeassistant/aarch64-homeassistant:$hav &> /dev/null
